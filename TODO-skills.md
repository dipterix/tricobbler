...
Add runtime configuration to the Skill R6 class so that the caller
(scheduler or user) can specify environment setup commands for script
execution. The skill itself does not know its runtime — the orchestrator
provides it.
...
- `runtime$python$setup` — shell command to run before `python3`
  (e.g. venv activation, conda activate)
...
3.  `build_script_command()` accepts optional `runtime` config:
    - **Python**: wraps in
      `bash -c "setup && python3 script.py args..."`
...
- A skill with `runtime$python$setup` activates the env before Python
  scripts
- A skill with no `runtime` works exactly as before (backward
  compatible)
...
The skill-creator scripts (init_skill.py, package_skill.py,
quick_validate.py) require `pyyaml`. Set up a project-level venv.
...
4.  Test: create Skill with runtime config pointing to the venv, call a
    script
...
1.  Load skill-creator into an ellmer::Chat with runtime pointing to
    .venv
...
R scripts use docopt for CLI parsing. Both R and Python versions
coexist.
